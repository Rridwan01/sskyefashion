  ...
  return (
    <>
      <motion.header
        ...
        className={`fixed top-0 inset-x-0 z-40 w-full flex items-center justify-between px-6 py-6 transition-colors duration-500 pointer-events-auto ${isHome && scrollY.get() < 800 ? "mix-blend-difference text-white" : "bg-background/80 backdrop-blur-md text-foreground border-b border-foreground/10"}`}
      >
      ...
      <AnimatePresence>
        {mobileMenuOpen && (
          <motion.div 
            ...
            exit={{ opacity: 0, y: -20 }}
            className="fixed inset-0 z-30 bg-background pt-24 px-6 flex flex-col pointer-events-auto"
          >
